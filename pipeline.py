...
def run_stage(
    name: str, indices: list[int], func, cleanup_dir: Path | None = None
) -> tuple[int, int, float]:
    success, failed = 0, 0
    start = time.time()
...
def main():
    ...
    stages = [
        (
            "Download",
            lambda i: download_video(i, str(rows[i - 1]["download_url"])),
            None,
        ),
        (
            "Trim",
            lambda i: trim_video(
                i,
                float(str(rows[i - 1]["trim_start"])),
                float(str(rows[i - 1]["trim_duration"])),
            ),
            None,
        ),
    ...
    summary.append(
        (
            "Concatenate",
            1 if concat_result else 0,
            0 if concat_result else 1,
            concat_elapsed,
        )
    )
